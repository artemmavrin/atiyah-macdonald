...
By \partref{1}{21}{iii}, \(V(\Ker(\phi)) = V((0)^c) = \bar{\phi^*(V(0))} = \bar{\phi^*(Y)}\)
...
Then \(X = \{(0), \mathfrak{p}\}\), where \((0)\) is a generic point since \(A\) is an integral domain, and \(\mathfrak{p}\) is a closed point by \expartref{1}{18}{i}.
...
In fact, both points of \(Y\) are maximal ideals of \(B\), so both are closed points by \expartref{1}{18}{i}, so the Zariski topology of \(Y\) is discrete.
